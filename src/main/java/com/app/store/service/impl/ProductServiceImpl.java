...
public class ProductServiceImpl implements ProductService {
    ...
    private final ProductRepository productRepository; //retrieve product data from the database
    private final com.app.store.repository.UserRepository userRepository;
    private final org.springframework.jdbc.core.JdbcTemplate jdbcTemplate;//MANUAL DATABASE query(to execute sql queries directly on the database)
    ...
    public Product createProduct(ProductDto productDto) {
        ...
        Product product = Product.builder() //create product object
    ...
    @Override
    public List<Product> searchProducts(String keyword) { // search function,product keywordsearch functionality matching products are display
    ...
    // selected category from drop down menu
    ...
    public List<Product> getProductsWithFilterAndSort(String keyword, String category, String sortOrder) {
        ...
        //combine filtering, sorting function
        
        if (keyword != null && !keyword.trim().isEmpty()) { // keyword filter, whether a search key word entered check
            spec = spec.and((root, query, cb) -> cb.or(
                    cb.like(cb.lower(root.get("title")), "%" + keyword.toLowerCase() + "%"),// search logic, filter product by matching product title with entered keyword
        ...
        } else if ("price_desc".equals(sortOrder)) { //price high to low
            sort = org.springframework.data.domain.Sort.by(org.springframework.data.domain.Sort.Direction.DESC,
                    "basePrice");
        } else if ("newest".equals(sortOrder)) { //newest product
            sort = org.springframework.data.domain.Sort.by(org.springframework.data.domain.Sort.Direction.DESC,
                    "createdAt");
        }

        return productRepository.findAll(spec, sort); //final database call
    }
}
